...
func CreateUser(c *gin.Context) {
	...
	rePwd := c.Request.FormValue("Identity")

	//salt是一个用于加密密码的随机字符串，通常与密码混合使用，以提高密码的安全性
	salt := fmt.Sprintf("%06d", rand.Int31())

	data := models.FindUserByName(user.Name)

	if user.Name == "" || pwd == "" || rePwd == "" {
		c.JSON(200, gin.H{
			"code":    -1, //  0成功   -1失败
			"message": "用户名或密码不能为空！",
			"data":    user,
	...
	user.PassWord = utils.MakePassword(pwd, salt)
	user.Salt = salt
	fmt.Println(user.PassWord)
	user.LoginTime = sql.NullTime{Time: time.Now(), Valid: true}
	user.LoginOutTime = sql.NullTime{Time: time.Now(), Valid: true}
	user.HeartbeatTime = sql.NullTime{Time: time.Now(), Valid: true}
...
func UpdateUser(c *gin.Context) {
	...
	//如果一个变量被声明但未被使用，会导致编译错误。使用空白标识符 _，我们可以在编译时避免这个错误，并显式地表明我们不需要使用该变量的值。
...
func MsgHandler(c *gin.Context, ws *websocket.Conn) {
	for {
		...
		tm := time.Now().Format("2023-07-02 15:04:05")
